...
<head>
  ...
  <meta charset="utf-8">
  <title>VOX DSP - Daily Statistics</title>
...
?>
    ...
    <section id="content_wrapper">
      ...
      <div class="panel-heading">
        <span class="panel-title"><b style="font-size:1.4em;">Daily Statistics - <?php echo $_GET['n']; ?></b></span>
      </div>
              ...
              <div id="tab2_1" class="tab-pane active">
                <div class="row">
                      ...
                      <table class="table table-striped table-hover" style="overflow:hidden;" id="datatable" cellspacing="0" width="100%">
                        <thead>
                          <tr>
                            <th data-toggle="tooltip" data-placement="bottom" title="<b>UTC Timestamp</b><br><i>This is the date in UTC.</i>" data-html="true">Date</th>
                            ...
                            <th data-toggle="tooltip" data-placement="bottom" title="<b>Total Cost</b><br><i>Price paid for the total banner displays.</i>" data-html="true">Cost</th>
                            <th data-toggle="tooltip" data-placement="bottom" title="<b>Effective Cost Per Mile</b><br><i>This is the average cost of 1,000 impressions for this campaign.</i>" data-html="true">eCPM</th>
    ...
    <?php
        ...
        $lcost = 0;
        $lclicks = 0;
        $logs = file_get_contents($logsdir.'dayreport/'.$_GET['cid'].'.txt');
        ...
        foreach($lines as $l)
        {
            ...
            $ecpm = divide($cost, divide($wins, 1000));
            echo '<tr>';
            echo '<td class="min"><b>'.$p[0].'</b></td>';
            echo '<td class="min">'.number_format($wins).'</td>';
            echo '<td class="min">'.number_format($cost, 2).'</td>';
            echo '<td class="min">'.number_format($ecpm, 2).'</td>';
            echo '<td class="min">'.number_format($clicks).'</td>';
            echo '<td class="min">'.number_format(divide(100, $wins)*$clicks, 2).'%</td>';
            echo '<td class="min">'.number_format(divide($cost, $clicks), 2).'</td>';
            echo '</tr>';
        ...
        //Add current day stats so far
        $wins = $redis->get('mb-'.$_GET['cid'].'-wins');
        $cost = $redis->get('mb-'.$_GET['cid'].'-cost');
        $clicks = $redis->get($_GET['cid'].'-clicks');

        $lswins = $redis->get('lsmb-'.$_GET['cid'].'-wins');
        $lscost = $redis->get('lsmb-'.$_GET['cid'].'-cost');
        $lsclicks = $redis->get('lsmb-'.$_GET['cid'].'-clicks');
        if(!$lswins)
            $lswins = 0;
        if(!$lscost)
            $lscost = 0;
        ...
        $today_cost = divide($cost-$lscost, 1000);
        $today_clicks = $clicks-$lsclicks;

        $ecpm = divide($today_cost, divide($today_wins, 1000));
        echo '<tr data-toggle="tooltip" data-placement="bottom" title="<b>These are the Live stats for today, these are not complete until the end of the day, UTC 00:00.</b>" data-html="true">';
        echo '<td class="min"><b><font color=#d9534f>'.date('Y-m-d').'</font></b></td>';
        echo '<td class="min"><b><font color=#d9534f>'.number_format($today_wins).'</font></b></td>';
        echo '<td class="min"><b><font color=#d9534f>'.number_format($today_cost, 2).'</font></b></td>';
        echo '<td class="min"><b><font color=#d9534f>'.number_format($ecpm, 2).'</font></b></td>';
        echo '<td class="min"><b><font color=#d9534f>'.number_format($today_clicks).'</font></b></td>';
        echo '<td class="min"><b><font color=#d9534f>'.number_format(divide(100, $today_wins)*$today_clicks, 2).'%</font></b></td>';
        echo '<td class="min"><b><font color=#d9534f>'.number_format(divide($today_cost, $today_clicks), 2).'</font></b></td>';
        echo '</tr>';
